...
# DERIVATIVE DIAGNOSTICS
    ...
    - **Slope (1st derivative)** indicates trend momentum.
    - **Acceleration (2nd derivative)** highlights shifts in momentum.
    - **Buy/Sell signals** are generated by thresholding these derivatives.
    """)
    ...
    entry_th   = st.sidebar.slider("Entry Threshold",  0.0, 2.0, 0.5, 0.1)
    exit_th    = st.sidebar.slider("Exit Threshold",  -2.0, 0.0, -0.5, 0.1)
    show_signals = st.sidebar.checkbox("Show Buy/Sell Points", value=True)
    ...
    if show_signals:
        ...
        fig.add_trace(go.Scatter(x=buys.index,  y=buys,  mode="markers", name="Buy",
                                 marker=dict(color="green", symbol="triangle-up", size=8)))
        fig.add_trace(go.Scatter(x=sells.index, y=sells, mode="markers", name="Sell",
