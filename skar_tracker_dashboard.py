...
st.title('📈 Universal Skar Tracking Signal Dashboard with Backtest')
...
stop_loss_pct = st.sidebar.slider('Trailing Stop Loss (%)', min_value=1, max_value=20, value=10)

@st.cache_data(ttl=300)
def get_data(ticker_symbol):
    df = yf.download(ticker_symbol, period='3y', progress=False)
    df = df[['Close']]
    df.columns = ['Price']
...
for i in range(len(data)):
    ...
    if position > 0:
        ...
        if signal < entry_threshold or price <= peak_price * (1 - stop_loss_pct / 100):
            cash = position * price
            position = 0
...
fig_portfolio.update_layout(title='Portfolio Value Over Time', xaxis_title='Date', yaxis_title='Value ($)', hovermode='x unified')
